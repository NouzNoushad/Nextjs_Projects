...
export default function AddTodo() {
    ...
    const { handleCreateTodo, isLoading, todoData } = AddTodoAction()
    return (
                    ...
                    <form onSubmit={handleCreateTodo} className="flex flex-col justify-between h-full">
                        <div className="space-y-4 flex-1">
                            <div className="space-y-1">
                                ...
                                <input type="text" name='title' placeholder='Enter the title' defaultValue={todoData && todoData.title} className='border-2 border-[#e7d2f7] w-full rounded-md px-2 py-2 outline-[#751aba]' />
                            </div>
                            <div className="space-y-1">
                                <label htmlFor="description" className='text-[0.95rem] font-bold'>Description</label>
                                <textarea rows={4} name='description' placeholder='Enter the description' defaultValue={todoData && todoData.description} className='border-2 border-[#e7d2f7] w-full rounded-md px-2 py-2 outline-[#751aba]' ></textarea>
                            </div>
                        </div>
                        <button type="submit" className='mt-5 inline-block w-full rounded-md bg-[#751aba] py-2 px-3 text-white font-[500] text-[0.98rem]'>
                            {
                                id ? (isLoading ? 'Updating...' : 'Update') : (isLoading ? 'Creating...' : 'Create Todo')
                            }
                        </button>
                    </form>
                </div>

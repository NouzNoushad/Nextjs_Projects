    ...
    return (
        <div>
            ...
            <div className={`mt-1 px-4 space-y-1 transition-transform ${showCatalog ? "overflow-y-auto" : "hidden overflow-y-hidden"}`}>
                ...
                {/* Categories */}
                <button onClick={() => handleCatalogChange(CatalogType.Categories, "/categories")} className={`nav-link group group-transition-colors group-duration-300 focus:outline-none focus:ring-0 w-full ${selectedCatalog == CatalogType.Categories ? "bg-primary-light" : "bg-transparent"}`}>
                    <span className={`block w-1 h-1 group-hover:bg-white rounded-full ${selectedCatalog == CatalogType.Categories ? "bg-white" : "bg-gray-400"}`}></span>
                    <span className={`text-[13px] font-normal group-hover:text-white ${selectedCatalog == CatalogType.Categories ? "text-white" : "text-gray-400"}`}>Categories</span>
                </button>
                {/* Add Products */}

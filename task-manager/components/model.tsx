...
export default function Model() {
    ...
    const { closeModal, resetTaskId } = useGlobalState()
    const { handleFormSubmit, errors, setCompleted, setImportant, completed, important, task } = ModalAction()

    return (
        <main className='fixed top-0 left-0 w-full h-screen z-[100]'>
            <div className="absolute top-0 left-0 w-full h-screen bg-[rgba(0,0,0,0.45)] backdrop-blur-sm flex items-center justify-center z-[10]" onClick={() => {
                resetTaskId()
                closeModal()
            }}>
                <div className="relative max-w-[630px] w-full px-[2rem] py-[2rem] bg-card z-[101] rounded-lg shadow-[0_0_1rem_rgba(0,0,0,0.3)]" onClick={(e) => e.stopPropagation()}>
                    ...
                    <form onSubmit={handleFormSubmit} className='space-y-2'>
                        <div className="space-y-1">
                            ...
                            <input type="text" name='title' className='w-full bg-[#181818] px-3 py-2 rounded-md' placeholder='Enter Title' defaultValue={task && task.title} />
                            {errors.title && <p className='form-error'>{errors.title}</p>}
                        </div>
                        <div className="space-y-1">
                            <label htmlFor="description" className='text-[0.9rem] font-[500]'>Description</label>
                            <textarea rows={4} name='description' className='w-full bg-[#181818] px-3 py-2 rounded-md' placeholder='eg. Watch a video' defaultValue={task && task.description}></textarea>

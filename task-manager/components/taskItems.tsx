                    ...
                    <div className="mt-4 grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-3">
                        {
                            tasks && tasks.map((task) => (
                                <div key={task.id} className="bg-card border-2 border-[#333] px-3 py-3 rounded-lg space-y-3">
                                    ...
                                    <div className="">
                                        <h3 className="text-[0.8rem]">{new Date(task.date).toLocaleDateString('en-US', {
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric'
                                        })}</h3>
                                        <div className="mt-1 flex flex-row items-center justify-between">
                                            <button className="text-[0.7rem] rounded-full bg-green-600 px-2 py-1">{task.completed ? 'completed' : 'Incompleted'}</button>

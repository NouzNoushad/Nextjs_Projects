        ...
        <header className='relative bg-blue-400'>
            <div className="w-[300px] fixed top-0 xl:left-0 left-[-100%] bottom-0 overflow-y-auto px-[15px] z-50 transition-all duration-300 primary-color border-r-4 border-white">
                <div className="flex flex-col items-center justify-between relative pt-5">
                    ...
                    <ul className={`lg:mt-8 flex flex-col items-center lg:items-stretch justify-center lg:justify-end gap-4 fixed top-0 primary-color h-full w-full lg:relative lg:top-0 lg:left-0 lg:bg-transparent z-50 transition-all duration-500 ${toggleMenu ? 'right-0' : 'right-[-100%]'}`}>
                        ...
                        <button onClick={() => handleToggle(selectedLink)} className={`inline-block lg:hidden border-2 border-white px-1 py-1 rounded-md drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.3)] fixed top-5 ${toggleMenu ? 'right-3' : 'right-[-100%]'}`}>
                            <CloseIcon className='size-5 text-white' />
                        </button>
                    </ul>
                </div>
            </div>
            <button onClick={() => handleToggle(selectedLink)} className="inline-block xl:hidden border-2 border-white px-1 py-1 rounded-md drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.3)] fixed top-5 right-3">
